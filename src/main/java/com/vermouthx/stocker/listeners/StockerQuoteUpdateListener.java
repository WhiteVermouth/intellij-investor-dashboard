    ...
    public void syncQuotes(List<StockerQuote> quotes, int size) {
        ...
        quotes.forEach(quote -> {
            synchronized (myTableView.getTableModel()) {
                ...
                if (rowIndex != -1) {
                    // Update existing row cell by cell. Compare against the exact value stored in
                    // the model (the Change% cell holds a "1.23%" string, not the raw Double) so
                    // unchanged cells are skipped instead of repainting on every refresh tick.
                    // DefaultTableModel.setValueAt fires the per-cell update event itself.
                    ...
                    // Column 8: Change%
                    String percentText = quote.getPercentage() + "%";
                    if (!percentText.equals(tableModel.getValueAt(rowIndex, 8))) {
                        tableModel.setValueAt(percentText, rowIndex, 8);
                    }
